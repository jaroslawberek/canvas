...
export class TileObject {
  ...
  selectTile(mouse) {
    ...
    this.selectedTile = {
      ...
      titleX: titleX,
      titleY: titleY,
      tableindex: this.table[titleY][titleX],
    };
  ...
  drawTileByIndex(ctx, image, index, tileW, tileH, posX, posY) {
    ...
    // Utils.cl(tileX)
    // Utils.cl(tileY)
    ctx.drawImage(image, tileX * tileW, tileY * tileH, tileW, tileH, posX, posY, tileW, tileH);
  }

  drawWithBuffer(ctx, mouseX, mouseY, buffer, width, height, tileW, tileH, camera = null) {
    let first = null;
...
*/
  ...
  static drawSelectedFill(ctx, x, y, titleSize, color = null) {
    if (!color) ctx.fillStyle = 'rgba(0,0,0,0.3)';
    else ctx.fillStyle = color;
    ctx.fillRect(x, y, titleSize, titleSize);
  }

  static getTitleCoordinate(x, y, tSize, camera = null) {
    // 🔹 uwzględnij zarówno pozycję kamery, jak i skalę
    const worldX = camera ? x / camera.scale + camera.x : x;
    const worldY = camera ? y / camera.scale + camera.y : y;

    const titleX = Math.floor(worldX / tSize);
    const titleY = Math.floor(worldY / tSize);
